                ...
                viewModelScope.launch {
                    ...
                    try {
                        if (state.isShareMode) {
                            if (state.originalPostId == null) throw Exception("Thiếu ID bài gốc")
                            ...
                            if (trimmed.isEmpty() && state.selectedImages.isEmpty()) {
                                emitEvent(CreatePostUiEvent.ShowMessage("Cần nội dung hoặc ảnh!"))
                                return@launch
                            }

                            // Upload media
                            val mediaItems = coroutineScope {
                                state.selectedImages.map { async(Dispatchers.IO) {
                                    val url = uploadToCloudinary(action.context, it.uri, uploadPreset)
                                    Media(type = if (url.contains("video")) "video" else "image", url = url)
                                } }.awaitAll()
                            }

                            // Tạo bài
                            val result = createPostUseCase(trimmed, state.privacy, mediaItems)
                            if (result.isSuccess) {
                                onAction(CreatePostAction.ShowNotice("Đăng thành công!", NoticeType.SUCCESS))
                                delay(1000)
                        ...
                        onAction(CreatePostAction.ShowNotice("Lỗi: ${e.message}", NoticeType.ERROR))
                    } finally {
